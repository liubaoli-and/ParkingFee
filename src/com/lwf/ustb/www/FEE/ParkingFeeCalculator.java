...
public class ParkingFeeCalculator
{
    ...
    private  double parkingFeeWithoutDiscount( int typeOfVehicle, int typeOfCar, int dayOfWeek, double apd )
    {
        ...
        if (typeOfVehicle != Vehicle.MOTORCYCLE && typeOfVehicle != Vehicle.CAR) {
            return -1;
        }
        if (typeOfVehicle == Vehicle.CAR && typeOfCar != Car.TWO_DOOR_COUPE && typeOfCar != Car.OTHERS) {
            return -1;
        }
        if (dayOfWeek != Day.WEEKDAY && dayOfWeek != Day.WEEKEND) {
            return -1;
        }
        int parkingHours;
        if (apd % 1 == 0) {
        ...
        case Day.WEEKDAY :
            if (parkingHours <= 2) {
                if (typeOfVehicle == Vehicle.MOTORCYCLE) {
                    fee = parkingHours * 4;
                } else {
                    if (typeOfCar == Car.TWO_DOOR_COUPE) {
    ...
    public static void main(String[] args){
    	...
    	System.out.println(p.parkingFee(0,0,1,1,false,null));
    }
